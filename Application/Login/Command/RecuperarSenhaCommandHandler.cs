...
namespace Application.Login.Command
{
    public class RecuperarSenhaCommandHandler : IRecuperarSenhaCommandHandler
    {
        ...
        public async Task Handle(string email)
        {
            var aluno = await _botuContext.Alunos.FirstOrDefaultAsync(u => u.Email == email);
            if (aluno == null)
            {
